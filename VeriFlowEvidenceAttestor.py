...
collateral-evidence pages. It stores a structured review result that can be
used by collateral-backed lending, treasury, or verification workflows.
"""
...
class VeriFlowEvidenceAttestor(gl.Contract):
    ...
    def __init__(self, review_policy: str):
    ...
    @gl.public.write
    def set_review_policy(self, review_policy: str) -> None:
        if gl.message.sender_address != self.owner:
            raise gl.vm.UserError("Only the contract owner can update the review policy")
        if len(review_policy.strip()) < 80:
            raise gl.vm.UserError("Review policy must be at least 80 characters")
        self.review_policy = review_policy

    @gl.public.write
    def review_public_collateral_evidence(self, subject: str, evidence_url: str) -> str:
        """Review an HTTPS public evidence page against the stored policy.

        The accepted decision is one of VERIFIED, INSUFFICIENT_EVIDENCE, or REJECTED.
        Only the consensus-agreed decision is persisted. Private KYC/AML data
        must never be included in the public evidence URL.
        """
        ...
        policy = gl.storage.copy_to_memory(self.review_policy)
